...
export default class S3Template {
  ...
  fetchTemplateFor(pageSlug) {
    let templateFilePromise;

    if (this.DEV_TEMPLATE_FOLDER) {
      templateFilePromise = RSVP.resolve(fs.readFileSync(path.join(this.DEV_TEMPLATE_FOLDER, `${pageSlug}.hbs`), 'utf8'));
    } else {
      templateFilePromise = this._compiler.fetchFromS3(`${pageSlug}.hbs`)
        .then(res => res.Body);
    }

    return templateFilePromise
      .then(body => {
